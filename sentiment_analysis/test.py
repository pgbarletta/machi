...
def check_hinge_loss_single():
    ...
    exp_res = 1 - 0.8
    if check_real(ex_name, p1.hinge_loss_single, exp_res, feature_vector,
                  label, theta, theta_0):
        return
    log(green("PASS"), ex_name, "")
...
def check_hinge_loss_full():
    ...
    exp_res = 1 - 0.8
    if check_real(ex_name, p1.hinge_loss_full, exp_res, feature_vector, label,
                  theta, theta_0):
        return
...
def check_perceptron_single_update():
    ...
    if check_tuple(ex_name, p1.perceptron_single_step_update, exp_res,
    ...
    if check_tuple(ex_name + " (boundary case)",
                   p1.perceptron_single_step_update, exp_res, feature_vector,
                   label, theta, theta_0):
        return
...
def check_perceptron():
    ...
    if check_tuple(ex_name, p1.perceptron, exp_res, feature_matrix, labels, T):
        return
...
def check_average_perceptron():
    ...
    T = 1
    exp_res = (np.array([1, 2]), 1)
    if check_tuple(ex_name, p1.average_perceptron, exp_res, feature_matrix,
                   labels, T):
        return
...
def check_pegasos_single_update():
    ...
    if check_tuple(ex_name, p1.pegasos_single_step_update, exp_res,
    ...
    if check_tuple(ex_name + " (boundary case)", p1.pegasos_single_step_update,
                   exp_res, feature_vector, label, L, eta, theta, theta_0):
        return
...
def check_pegasos():
    ...
    L = 0.2
    exp_res = (np.array([1, 2]), 1)
    if check_tuple(ex_name, p1.pegasos, exp_res, feature_matrix, labels, T, L):
        return
...
def check_classify():
    ...
    if check_array(ex_name, p1.classify, exp_res, feature_matrix, theta,
                   theta_0):
        return
    ...
    theta_0 = 0
    exp_res = np.array([-1])
    if check_array(ex_name + " (boundary case)", p1.classify, exp_res,
